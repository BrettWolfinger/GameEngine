...
void PongGame::updateBall(float dt) {
    ...
    if (m_ball.y < 0.f)             { m_ball.y = 0.f;             m_ball.vy = -m_ball.vy; Engine::Audio::playTone(240.f, 0.04f); }
    if (m_ball.y + m_ball.size > H) { m_ball.y = H - m_ball.size; m_ball.vy = -m_ball.vy; Engine::Audio::playTone(240.f, 0.04f); }
    ...
    if (m_ball.x + m_ball.size < 0.f) {
        ++m_scoreRight;
        Engine::Audio::playTone(120.f, 0.3f);
